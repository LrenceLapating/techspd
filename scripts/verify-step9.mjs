...
const checks = [
  ...
  },
  {
    ...
    patterns: [
      "[TechSpd Latency] webhook received",
      "webhookReceivedAt",
      "webhook_to_database_ms",
    ],
  },
  {
    file: "src/lib/meta/webhook.ts",
    patterns: [
      'webhook_received_at: webhookReceivedAt',
      '.select("id, created_at")',
      "[TechSpd Latency] message inserted",
      "webhookToDatabaseMs",
      "findExistingMessageContext",
      "contextLookupMs",
      "fastPath: Boolean(existingContext)",
      "profilePromise",
      "await Promise.all([",
    ],
    absent: ["const profile = await fetchFacebookCustomerProfile"],
  },
  {
